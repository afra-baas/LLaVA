...
class LlavaMetaForCausalLM(ABC):
    ...
    def prepare_inputs_labels_for_multimodal(
        ...
        embedding_size= concat_features.size(-1)
        fused_embedding_size= image_features.size(-1)
        # print('embedding_size', embedding_size, type(embedding_size)) #8192
        # print('fused_embedding_size', fused_embedding_size) #4096
        fusion_layer = torch.nn.Linear(embedding_size, fused_embedding_size).to(image_features.device).to(input_dtype)
        # fusion_layer = torch.nn.Linear(embedding_size*2, fused_embedding_size)
        # learnable_fused_embedding = fusion_layer(concat_features)
        image_features = fusion_layer(concat_features)
