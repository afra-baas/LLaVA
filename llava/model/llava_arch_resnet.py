...
class LlavaMetaForCausalLM(ABC):
    ...
    def encode_images(self, images, depth_images=None):
        ...
        for name, param in self.depth_cnn.named_parameters(): # during training all have dtype bfloat16, during eval float16, whyy???
            if name=='cnn.0.weight':
                print(f"Parameter {name}: dtype={param.dtype}")
                depth_cnn_dtype=param.dtype
                break
        # depth_cnn_dtype=param.dtype

        depth_images = depth_images.to(depth_cnn_dtype)
        depth_features = self.depth_cnn(depth_images)

        image_features = self.get_model().get_vision_tower()(images)
        image_features = image_features.to(depth_cnn_dtype)

        # print("image_features na alleen vision encoder", image_features.shape) # torch.Size([1, 576, 1024]) 
        # print("depth image_features na resnet", depth_features.shape) # torch.Size([16, 1, 1024])  

        # concatenated_embedding = torch.cat((depth_features, image_features), dim=1)
        # print('concatenated_embedding', concatenated_embedding.shape) #[32, 577, 1024]

        # image_features = self.get_model().mm_projector(concatenated_embedding)

        # image_features = self.fusion_module(concatenated_embedding)


        image_features = self.get_model().mm_projector(image_features)
        depth_image_features = self.get_model().mm_projector(depth_features)
        print("image_features na projector", image_features.shape) #[32, 576, 4096]
        print("image_features na projector", depth_image_features.shape) #[32, 576, 4096]

        concatenated_embedding = torch.cat((image_features, depth_image_features), dim=1)
    ...
    def prepare_inputs_labels_for_multimodal(
        ...
        print("prepare_inputs_labels_for_multimodal in LlavaMetaForCausalLM in arch resnet")
        ...
        if type(images) is list or images.ndim == 5:
            if type(images) is list:
                print('need extra check from arch in arch resnet')
