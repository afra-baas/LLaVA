...
from llava.model.language_model.llava_llama_resnet import LlavaLlamaForCausalLM, AutoModelForCausalLM, LlavaConfig
from llava.train.train_for_resnet import *
...
class DepthCNN(nn.Module):
    def __init__(self, embedding_size):
        ...
        # Add linear layer for embedding
        self.fc = nn.Linear(num_features, embedding_size)
    ...
    def forward(self, x):
        ...
        depth_features = embedding.unsqueeze(1)#.expand(-1, self.num_image_features, -1)
...
class DepthLlavaLlamaForCausalLM(LlavaLlamaForCausalLM):
    ...
    def __init__(self, config): #, device=None -> error: TypeError: Object of type device is not JSON serializable  
        ...
        depth_embedding_size = 1024 #128  # Size of the depth map embedding
        self.depth_cnn = DepthCNN(embedding_size=depth_embedding_size)
        # self.fusion_module = FusionModule(input_size=577 * 1024, output_size=1024)  # Input size is the sum of the embedding sizes
        # self.fusion_module = FusionModule(input_size= 1024, output_size=1024)  # Input size is the sum of the embedding sizes
