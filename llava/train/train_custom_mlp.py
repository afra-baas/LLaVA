...
class DepthLlavaLlamaForCausalLM(LlavaLlamaForCausalLM):
    ...
    def __init__(self, config):
        ...
        self.input_channels=input_channels = 4
        self.output_channels=output_channels = 3
        self.flatten = nn.Flatten()
        self.fc1 = nn.Linear(input_channels * 336 * 336, 512)  # Adjust the hidden size as needed
        self.fc1.requires_grad_(True)
        self.relu = nn.ReLU()
        self.fc2 = nn.Linear(512, output_channels * 336 * 336)
        self.fc2.requires_grad_(True)
        print('state_dict fc1 init', self.fc1.state_dict()) 
        print('state_dict fc2 init', self.fc2.state_dict()) 
    ...
    def forward(
        ...
        if inputs_embeds is None:
            if images is not None and depth_images is not None:
                ...
                print("image and depth map concatenated before encoder")
            (
                input_ids,
                position_ids,
                attention_mask,
    ...
    def encode_images(self, images):
        # print('img shape', images.shape) #([32, 4, 336, 336])
        # print(images.dtype)
        # print(self.fc1.weight.dtype) #bfloat16
        # print(self.fc2.weight.dtype)

        x = self.flatten(images)
        # print("flat img shape", x.shape) #[1, 451584])
        # print(type(x))
        ...
        images = x.view(-1, self.output_channels, 336, 336)
        # print('img shape', images.shape) #([32, 3, 336, 336])
...
def train():
    ...
    weight_has_issue = torch.isnan(fc1_state_dict['base_layer.weight']).any() or torch.isinf(fc1_state_dict['base_layer.weight']).any()
    bias_has_issue = torch.isnan(fc1_state_dict['base_layer.bias']).any() or torch.isinf(fc1_state_dict['base_layer.bias']).any()
    print("Weight has NaN, -inf, or inf:", weight_has_issue)
    print("Bias has NaN, -inf, or inf:", bias_has_issue)

    weight_has_issue = torch.isnan(fc2_state_dict['base_layer.weight']).any() or torch.isinf(fc2_state_dict['base_layer.weight']).any()
    bias_has_issue = torch.isnan(fc2_state_dict['base_layer.bias']).any() or torch.isinf(fc2_state_dict['base_layer.bias']).any()
    print("Weight has NaN, -inf, or inf:", weight_has_issue)
    print("Bias has NaN, -inf, or inf:", bias_has_issue)
    

    print('state_dict fc1 before save', fc1_state_dict) 
    print('state_dict fc2 before save', fc2_state_dict) 

    print('weight path', training_args.output_dir)
    torch.save(fc1_state_dict, os.path.join(training_args.output_dir, 'fc1_model.pth'))
    torch.save(fc2_state_dict, os.path.join(training_args.output_dir, 'fc2_model.pth'))
