...
class LazySupervisedDataset(Dataset):
    ...
    def __init__(self, data_path: str,
                 tokenizer: transformers.PreTrainedTokenizer,
                 data_args: DataArguments,
                 processor: transformers.InstructBlipProcessor):
    ...
    def __getitem__(self, idx):
        ...
        # max_length =2048
        max_length =512
        encodings = self.processor(images = image, text = prompt, padding = "max_length",max_length=max_length, return_tensors = "pt")#.to(model.device, torch.float16)
        labels = self.processor(text = answer, padding = "max_length",max_length=max_length, return_tensors = "pt").input_ids

        encodings['labels'] = labels
        encodings = {k:v.squeeze() for k,v in encodings.items()}
        # print(encodings.keys())
        # data_dict = dict(input_ids=input_ids, labels=targets, image= image)
        return encodings
