...
CHUNKS=${#GPULIST[@]}
CKPT="epoch10-nodepth-llava-v1.5-13b"
...
for IDX in $(seq 0 $((CHUNKS-1))); do
    ...
    CUDA_VISIBLE_DEVICES=${GPULIST[$IDX]} python -m llava.eval.model_vqa_science \
        ...
        --image-folder '' \
        --answers-file /project/forked_repos/LLaVA/playground/data/eval/custom2/answers_folder/whatsup/$CKPT/${CHUNKS}_${IDX}.jsonl \
        --num-chunks $CHUNKS \
        --chunk-idx $IDX \
        --temperature 0 \
        --conv-mode vicuna_v1 &
...
output_file=/project/forked_repos/LLaVA/playground/data/eval/custom2/answers_folder/whatsup/$CKPT/merge.jsonl
...
for IDX in $(seq 0 $((CHUNKS-1))); do
    cat /project/forked_repos/LLaVA/playground/data/eval/custom2/answers_folder/whatsup/$CKPT/${CHUNKS}_${IDX}.jsonl >> "$output_file"
done
...
python scripts/convert_custom_for_submission.py \
    ...
    --result-upload-file /project/forked_repos/LLaVA/playground/data/eval/custom2/answers_folder/whatsup/${CKPT}/predictions.jsonl
