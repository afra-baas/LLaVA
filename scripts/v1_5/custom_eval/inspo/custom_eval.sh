...
for IDX in $(seq 0 $((CHUNKS-1))); do
    CUDA_VISIBLE_DEVICES=${GPULIST[$IDX]} python -m llava.eval.model_vqa_science \
        --model-path ./checkpoints/checkpoint-epoch10-llava-v1.5-13b-lora \
        --model-base liuhaotian/llava-v1.5-13b \
        --question-file ./playground/data/eval/custom/VSR_test_classification.json \
        --image-folder '' \
...
python scripts/convert_custom_for_submission.py \
    --annotation-file ./playground/data/eval/custom/VSR_test_classification.json \
