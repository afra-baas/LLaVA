...
CHUNKS=${#GPULIST[@]}
CKPT="llava-v1.5-13b_pt_front_behind"
...
for IDX in $(seq 0 $((CHUNKS-1))); do
    CUDA_VISIBLE_DEVICES=${GPULIST[$IDX]} python -m llava.eval.model_vqa_science \
        --model-path ./checkpoints/checkpoint-baseline-llava-v1.5-13b-lora_front_behind \
        --model-base liuhaotian/llava-v1.5-13b \
        --question-file ./playground/data/eval/custom/COCO_test_classification_front_behind.json \
        --image-folder '' \
        --answers-file ./playground/data/eval/custom/answers_custom/$CKPT/${CHUNKS}_${IDX}_pt_front_behind_test.jsonl \
        --num-chunks $CHUNKS \
        --chunk-idx $IDX \
        --temperature 0 \
        --conv-mode vicuna_v1 &
...
output_file=./playground/data/eval/custom/answers_custom/$CKPT/merge_pt_front_behind_test.jsonl
...
for IDX in $(seq 0 $((CHUNKS-1))); do
    cat ./playground/data/eval/custom/answers_custom/$CKPT/${CHUNKS}_${IDX}_pt_front_behind_test.jsonl >> "$output_file"
done
...
python scripts/convert_custom_for_submission.py \
    ...
    --result-upload-file ./playground/data/eval/custom/answers_upload/${CKPT}_pt_front_behind_test.jsonl
